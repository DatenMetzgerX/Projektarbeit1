 ...
 *
 * @property {function (first: L, others: L[]): L} joinBranches joins the out values from the predecessor nodes and returns the in value for the following node.
 ...
 * @property {function (node: ASTNode, inState: L): L} transfer determines the out state for the passed in node with the given in state
 * @property {function (x: L, y: L): true} areLatticesEqual determines if the two lattices are equal or not
 */

/**
 * Worklist algorithm for traversing the cfg.
 */
export class WorklistDataFlowAnalyzer {
	...
	analyze() {
		...
		// most probably it is preferred to first start with all entry nodes and then proceed with non entry nodes
	...
	initialize() {
		for (const node of this.cfg.getNodes()) {
			let inLattice = node.predecessors.size === 0 ? this.analisis.createEntryLattice() : this.analisis.createEmptyLattice();
	...
	joinInSetsFromPredecessors(node) {
		...
		if (tail.length === 0) {
			node.annotation.in = head;
		} else {
			node.annotation.in = this.analisis.joinBranches(head, tail, node);
		}
	}
}
