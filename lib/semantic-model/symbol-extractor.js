...
class SymbolExtractorVisitor {
	...
	identifier (path) {
		return this._registerIdentifier(path);
	}
	...
	assignmentExpression (path) {
		...
		if (target.isMemberExpression()) {
			return this._registerMemberSymbol(target, path.node);
	...
	functionDeclaration (path) {
		if (path.node.id) {
			...
			path.skipKey("id");
			return this.identifier(path.get("id"));
		}
	}

	objectExpression (path) {
		const objectSymbol = this._getObjectSymbol(path);

		for (const property of path.node.properties) {
			const symbol = new Symbol(property.key.name, SymbolFlags.Property);
			objectSymbol.addMember(symbol);
		}
	}

	_getObjectSymbol(objectExpression) {
		if (objectExpression.parentPath.isVariableDeclarator()) {
			return this.identifier(objectExpression.parentPath.get("id"));
		}

		if (objectExpression.parentPath.isAssignmentExpression()) {
			return this.assignmentExpression(objectExpression.parentPath);
		}

		if (objectExpression.parentPath.isObjectProperty()) {
			const parentObject = this._getObjectSymbol(objectExpression.parentPath.parentPath);
			return parentObject.getMember(objectExpression.parent.key.name);
		}

		assert.fail("Object for object expression could not be determined");
	}
...
const visitor = {
	...
	ObjectExpression (path) {
		this.internalVisitor.objectExpression(path);
	},
