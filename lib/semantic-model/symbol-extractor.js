...
export class SymbolExtractor {
	...
	enterReturnStatement(path) {
		...
		this.program.symbolTable.setSymbol(path.node, this.scope.resolveSymbol("return"));
	}
	...
	enterFunctionDeclaration(path) {
		...
		this.scope.addSymbol(new Symbol("return", SymbolFlags.Return));
		this.scope.addSymbol(new Symbol("this", SymbolFlags.Variable));
	...
	enterFunctionExpression(path) {
		assert(path.node.id, "Only function expressions with an id are supported");
