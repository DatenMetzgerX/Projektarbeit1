...
export class TypeEnvironment {
	...
	merge(...environments) {
		...
		const mappings = [...environments].map(env => env._mappings);
		const merged = this._mappings.mergeWith((old, newV) => {
			assert.fail(`there are multiple in values of different branches ${old}, ${newV}, not yet supporrted`);
		}, ...mappings);

		if (merged === this._mappings) {
			return this;
		}

		return new TypeEnvironment(merged);
	}
	...
	dump(stream) {
		this._mappings.forEach((type, symbol) => {
			stream.write(`${symbol.name} -> ${type.resolveDeep()}\n`);
		});
	}
}
