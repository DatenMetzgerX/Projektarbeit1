...
export class CallExpressionRefinementRule {
	...
	/**
	 * Handle a function that has been defined externaly, therefor the body of the function is unkonwn. But the signature
	...
	_handleExternalDeclaredFunction(functionType, callExpression, callerContext) {
		...
		for (let i = 0; i < functionType.params.length; ++i) {
			...
			if (parameterType instanceof FunctionType && argumentType instanceof FunctionType && argumentType.declaration) {
	...
	_invokeFunction(functionType, invocation, callerContext) {
		...
		callContext.setType(Symbol.THIS, invocation.thisType);
	...
	_updateTypes(invocation, callerContext) {
		...
		callerContext.replaceTypes(invocation.context, [Symbol.RETURN, Symbol.THIS]);
	}
