...
import {RecordType, ObjectType, NullType, MaybeType, VoidType, TypeVariable} from "../semantic-model/types";
...
export class HindleyMilnerContext {
	...
	getObjectType(node) {
		...
		if (node.object.type === "Identifier") {
			...
			objectType = parentObjectType.getType(parentObjectSymbol.getMember(node.object.property.name)) || VoidType.create();
		} else if (t.isLiteral(node.object)) { // e.g. "test".length
			objectType = this.infer(node.object);
		} else {
			assert.fail(`Node type ${node.object.type} for the object of a member expression not yet supported.`);
		}
