 ...
 * The hindley milner algorighm uses a set of rules that perform the refinement. By default, the refinment rules
 * are loaded from the './refinment-rules' directory. Each rule must default export the Rule-Class.
...
export class HindleyMilner {
	...
	unify(t1, t2, node) {
		...
		try {
			return this.unificator.unify(t1, t2);
		} catch (e) {
	...
	_getRefinementRule(node) {
		...
		default:
			throw new TypeInferenceError(`The refinement rule to use for a node of type ${node.type} is ambiguous.`, node);
		}
	}
}
