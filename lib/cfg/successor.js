 ...
 * @param {!NodePath} path the path for which the nearest parent should be found
...
function computeBreakSuccessor (breakPath) {
	...
	if (breakPath.node.label) {
		parentToBreak = getParent(breakPath, path => path.isLabeledStatement() && path.node.label.name === breakPath.node.label.name);
	} else {
		parentToBreak = getParent(breakPath, path => path.isLoop() || path.isSwitchStatement());
	}

	assert(parentToBreak, "break statements outside of a loop or switch statement or with a label without a matching labeled block statement are not valid");
...
export function computeSuccessor (path) {
	...
	if (path.isContinueStatement()) {
		const parentLoop = getParent(path, loop => loop.isLoop() && (!path.node.label || (loop.parentPath.node.label && loop.parentPath.node.label.name === path.node.label.name)));
