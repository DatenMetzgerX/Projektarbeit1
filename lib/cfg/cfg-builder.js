...
const cfgVisitor = {
	Statement(path) {
		...
		if (path.isExpressionStatement() || path.isVariableDeclaration() || path.isEmptyStatement() || path.isContinueStatement() || path.isBreakStatement()) {
...
export function createControlFlowGraph(ast) {
	const cfg = new ControlFlowGraph(ast.program);

	traverse(ast, cfgVisitor, null, { cfg: cfg, exceptionHandlers: [] });
	return cfg;
}
