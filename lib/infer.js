...
import TypeInferenceDataFlowAnalysis from "./data-flow-analysis/type-inference-data-flow-analysis";
...
export function infer (file, context, program) {
	...
	plotControlFlowGraph(cfg);

	const dataFlow = new WorklistDataFlowAnalyzer(cfg, TypeInferenceDataFlowAnalysis);
	dataFlow.analyze();

	console.log(cfg.getNode(null).annotation.out.toArray());
